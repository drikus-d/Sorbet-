...
combined_formatted as (
    SELECT 
        ...
        sum(units_current) AS units_current,
        sum(units_py) AS units_py,
        TO_VARCHAR(sum(case when revenue_py = 0 then null else ROUND(((revenue_current/revenue_py)-1)*100,1) end)) || '%' as rand_growth,
        TO_VARCHAR(sum(case when units_py = 0 then null else ROUND(((units_current/units_py)-1)*100,1) end)) || '%' as unit_growth,
        cat_order,
        sub_cat_order
    FROM combined
    GROUP BY ALL
...
select 
    sub_category as "Retail Top 20 Items",
    revenue_current AS "Total Revenue Current",
    revenue_py AS "Total Revenue PY",
    units_current AS "Total Units Current",
    units_py AS "Total Units PY",
