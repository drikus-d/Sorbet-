...
summary as (
    ...
    select 'Last Year %' as act_bud,
            -- TO_VARCHAR(ROUND((sum(iff(act_bud = 'Actual', wtd, 0)) / sum(iff(act_bud = 'Last Year', wtd, 0)))-1,2)) || '%' as wtd,
            -- TO_VARCHAR(ROUND((sum(iff(act_bud = 'Actual', mtd, 0)) / sum(iff(act_bud = 'Last Year', mtd, 0)))-1,2)) || '%' as mtd,
            -- TO_VARCHAR(ROUND((sum(iff(act_bud = 'Actual', ytd, 0)) / sum(iff(act_bud = 'Last Year', ytd, 0)))-1,2)) || '%' as ytd
